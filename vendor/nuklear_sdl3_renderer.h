...
NK_INTERN void nk_sdl_device_upload_atlas(struct nk_context* ctx,
...
NK_API void nk_sdl_update_TextInput(struct nk_context* ctx) {
  ...
  if (active != sdl->edit_was_active) {
    ...
    NK_ASSERT(window_edit_active == sdl->edit_was_active &&
              "something else changed TextInputActive state for this Window");
   ...
   * bounds and the text cursor offset, and pass this data into
   * SDL_SetTextInputArea. This is currently not possible to do safely as
   * Nuklear does not support it.
    ...
    nk_draw_foreach(cmd, &sdl->ctx, &sdl->ogl.cmds) {
...

      {
        ...
        SDL_RenderGeometryRaw(
            sdl->renderer, (SDL_Texture*)cmd->texture.ptr,
...
NK_INTERN void nk_sdl_clipboard_copy(nk_handle usr, const char* text, int len) {
  ...
  /* FIXME: there is a bug in Nuklear that affects UTF8 clipboard handling
   * "len" is expected to be a buffer length, but due to bug it actually is a
   * glyph count see: https://github.com/Immediate-Mode-UI/Nuklear/pull/841 */
#if 0
    buflen = len + 1;
    NK_UNUSED(ptext);
#else
  ptext = text;
  for (i = len; i > 0; i--) (void)SDL_StepUTF8(&ptext, NULL);
...
NK_API struct nk_context* nk_sdl_init(SDL_Window* win, SDL_Renderer* renderer,
  ...
  nk_buffer_init(&sdl->ogl.cmds, &sdl->allocator,
...
NK_API void nk_sdl_font_stash_end(struct nk_context* ctx) {
  ...
  nk_font_atlas_end(&sdl->atlas, nk_handle_ptr(sdl->ogl.font_tex),
                    &sdl->ogl.tex_null);
...
NK_API int nk_sdl_handle_event(struct nk_context* ctx, SDL_Event* evt) {
  ...
  switch (evt->type) {
    ...
    case SDL_EVENT_MOUSE_BUTTON_DOWN: {
      ...
      const double dt =
          (double)(evt->button.timestamp - sdl->last_left_click) / 1000000000.0;
      switch (evt->button.button) {
        case SDL_BUTTON_LEFT:
          nk_input_button(ctx, NK_BUTTON_LEFT, x, y, down);
          nk_input_button(ctx, NK_BUTTON_DOUBLE, x, y,
                          down && dt > NK_SDL_DOUBLE_CLICK_LO &&
                              dt < NK_SDL_DOUBLE_CLICK_HI);
    ...
    case SDL_EVENT_MOUSE_MOTION:
      ...
      ctx->input.mouse.delta.x =
          ctx->input.mouse.pos.x - ctx->input.mouse.prev.x;
      ctx->input.mouse.delta.y =
...
#ifdef NK_INCLUDE_FONT_BAKING
  if (sdl->atlas.font_num > 0) nk_font_atlas_clear(&sdl->atlas);
#endif
...
NK_INTERN float nk_sdl_query_debug_font_width(nk_handle handle, float height,
                                              const char* text, int len) {
  NK_UNUSED(handle);
  return nk_utf_len(text, len) * height;
}

NK_INTERN void nk_sdl_query_debug_font_glypth(nk_handle handle, float height,
...
NK_API void nk_sdl_style_set_debug_font(struct nk_context* ctx) {
  ...
  surface = SDL_CreateSurface(NK_SDL_DFWH * SDL_DEBUG_TEXT_FONT_CHARACTER_SIZE,
  ...
  for (x = 0; x < NK_SDL_DFWH; x++) {
    for (y = 0; y < NK_SDL_DFWH; y++) {
      success = SDL_RenderDebugText(
          renderer, (float)(x * SDL_DEBUG_TEXT_FONT_CHARACTER_SIZE),
          (float)(y * SDL_DEBUG_TEXT_FONT_CHARACTER_SIZE), buf);
      NK_ASSERT(success);
      buf[0]++;
