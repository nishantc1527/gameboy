...
static void do_scanline_cgb(struct Ppu* ppu, Mmu* mmu) {
  uint8_t ly = mmu_r_mem(mmu, LY);
  uint8_t lcdc = mmu_r_mem(mmu, LCDC);
  ...
  }
  {
    ...
    for (int x = 0; x < SCRN_WIDTH; x++) {
      uint8_t by = (uint8_t)((ly + mmu_r_mem(mmu, SCY)) % 256);
      uint8_t tiley = by / 8;
      int offy = by % 8;
      ...
      uint8_t lo = mmu_get_bg_pal_byte(mmu, (uint8_t)(pal_num * 8 + clr * 2));
      uint8_t hi =
          mmu_get_bg_pal_byte(mmu, (uint8_t)(pal_num * 8 + clr * 2 + 1));
    ...
    if (get_bit(lcdc, 5)) {
      ...
      if (wx < SCRN_WIDTH + 7 && wy < SCRN_HEIGHT && ly >= wy) {
        ...
        for (uint8_t x = wx; x < SCRN_WIDTH; x++) {
          ...
          bg_prio_bit[x] = bg_prio;
          uint8_t lo =
              mmu_get_bg_pal_byte(mmu, (uint8_t)(pal_num * 8 + clr * 2));
          uint8_t hi =
              mmu_get_bg_pal_byte(mmu, (uint8_t)(pal_num * 8 + clr * 2 + 1));
          ppu->cgb_dsp[ly][x] = (uint16_t)(lo | ((uint16_t)hi << 8));
        }
  ...
  if (get_bit(lcdc, 1)) {
    ...
    for (int i = cnt - 1; i >= 0; i--) {
      ...
      uint16_t tile_idx = mmu_r_mem(mmu, mem_loc + 2);
      ...
      for (int x0 = x; x0 < x + 8; x0++) {
        ...
        if (!bg_wins) {
          uint8_t lo =
              mmu_get_obj_pal_byte(mmu, (uint8_t)(pal_num * 8 + clr * 2));
          uint8_t hi =
              mmu_get_obj_pal_byte(mmu, (uint8_t)(pal_num * 8 + clr * 2 + 1));
