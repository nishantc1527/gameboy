...
struct Bus* bus_init(struct Mmu* mmu, struct Cpu* cpu, struct Ppu* ppu,
  ...
  new_bus->timer = timer;
  new_bus->dma = dma;
  new_bus->joypad = joypad;
  new_bus->serial = serial;
  return new_bus;
}

void bus_free(struct Bus* bus_ptr) { free(bus_ptr); }
...
static uint8_t io_read(struct Bus* bus, uint16_t addr) {
  ...
  if (addr == 0xFF01 || addr == 0xFF02) {
    return serial_read(bus->serial, addr);
  }
  if (addr >= 0xFF04 && addr <= 0xFF07) {
    return timer_read(bus->timer, addr);
  }
  if (addr == 0xFF0F) {
    return cpu_read_if(bus->cpu);
  }
  ...
  if (addr >= 0xFF51 && addr <= 0xFF55) {
    return dma_hdma_read(bus->dma, addr);
  }
  if (addr >= 0xFF68 && addr <= 0xFF6B) {
    return ppu_read(bus->ppu, addr);
  }
  if (addr == 0xFF4D) {
    return mmu_read_cgb_speed(bus->mmu);
  }
  if (addr == 0xFF4F) {
    return mmu_get_vram_bank(bus->mmu);
  }
  if (addr == 0xFF70) {
    return mmu_get_wram_bank(bus->mmu);
  }
  return BUS_OPEN_BUS;
}
...
uint8_t bus_read(struct Bus* bus, uint16_t addr) {
  if (dma_blocks_cpu(bus->dma) && addr < HRAM_START) {
    return BUS_OPEN_BUS;
  }
  ...
  if (addr < OAM_START) {
    return mmu_read_wram(bus->mmu, (uint16_t)(addr - ECHO_OFFSET));
  }
  if (addr < OAM_END + 1) {
    return bus_read_oam(bus, addr);
  }
  if (addr < IO_START) {
    return BUS_OPEN_BUS;
  }
  if (addr < HRAM_START) {
    return io_read(bus, addr);
  }
  if (addr == IE_REG_ADDR) {
    return cpu_read_ie(bus->cpu);
  }
...
void bus_write(struct Bus* bus, uint16_t addr, uint8_t val) {
  ...
  if (addr < OAM_START) {
    mmu_write_wram(bus->mmu, (uint16_t)(addr - ECHO_OFFSET), val);
    return;
  }
  if (addr < OAM_END + 1) {
